    ...
    <div className="pt-24 pb-16">
      <div className="mx-auto max-w-5xl px-5">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-display font-bold text-[#CFC89A]">Flashcards</h1>
        ...
        {cards.length === 0 ? (
          <div className="card p-12 text-center text-[#CFC89A]/30">No flashcards yet.</div>
        ) : (
          <div className="grid gap-3">
            {cards.map((c) => (
              <div key={c._id} className="card p-4 flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-[#CFC89A] font-medium truncate">{c.front}</p>
                  <p className="text-[#CFC89A]/40 text-xs truncate mt-0.5">{c.back}</p>
                </div>
