  ...
  return (
    <div className="pt-24 pb-16">
      <div className="mx-auto max-w-6xl px-5">
        ...
        <h1 className="text-2xl font-display font-bold text-[#CFC89A] mb-8">Quizzes: <span className="text-amber">{slug}</span></h1>

        <div className="grid lg:grid-cols-[400px_1fr] gap-6">
          <div className="card p-5 lg:sticky lg:top-24 self-start">
            <h2 className="font-semibold text-[#CFC89A] mb-4">{editingId ? 'Edit Quiz' : 'New Quiz'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-[#CFC89A]/50 mb-1 block">Questions (one per line: Question|Opt1|Opt2|Opt3|AnswerIndex)</label>
          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <div className="card text-center py-12"><p className="text-2xl mb-2">📭</p><p className="text-[#CFC89A]/30 text-sm">No quizzes yet.</p></div>
            ) : (
              quizzes.map((q) => (
                <div key={q._id} className="card p-4 flex items-center justify-between">
                  <div>
                    <p className="font-medium text-sm text-[#CFC89A]">{q.title}</p>
                    <p className="text-xs text-[#CFC89A]/40">{q.questions?.length || 0} questions · {q.level}</p>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => { setEditingId(q._id); setForm({ title: q.title, level: q.level, questions: q.questions.map((qq) => `${qq.prompt}|${qq.options.join('|')}|${qq.answer}`).join('\n') }); }} className="p-1.5 rounded-lg text-[#CFC89A]/30 hover:text-amber hover:bg-amber/10 transition-all">
                    ...
                    <button onClick={async () => { if (!window.confirm('Delete?')) return; try { await api.delete(`/api/admin/quizzes/${q._id}`); fetchQuizzes(); toast.success('Deleted'); } catch { toast.error('Failed'); } }} className="p-1.5 rounded-lg text-[#CFC89A]/30 hover:text-rust hover:bg-rust/10 transition-all">
