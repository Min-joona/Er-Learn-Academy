      ...
      <div className="mx-auto max-w-7xl px-5">
          ...
          {categories.map((c) => (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                cat === c.id ? 'bg-amber text-white shadow-lg shadow-amber/20' : 'bg-[#ECE5CE]/5 text-[#ECE5CE]/50 hover:bg-[#ECE5CE]/10 hover:text-[#ECE5CE] border border-[#ECE5CE]/10'
              }`}
            >
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-[#ECE5CE]/50">No courses in this category yet.</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {courses.map((c) => (
              <Link key={c.slug} to={`/courses/${c.slug}`} className="card-hover group overflow-hidden p-0">
                ...
                <div className="p-5">
                  <h3 className="font-display font-bold text-lg text-[#ECE5CE] group-hover:text-amber transition-colors">
                    {c.title} <span className="text-sm font-normal text-[#ECE5CE]/30">{c.titleTi}</span>
                  </h3>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    ...
                    {(c.instructionLanguages || []).slice(0, 2).map((l) => (
                      <span key={l} className="pill bg-[#ECE5CE]/10 text-[#ECE5CE]/40 text-[10px]">{l}</span>
                    ))}
                  </div>
                </div>
              </Link>
