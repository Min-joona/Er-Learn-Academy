...
function Tester({ drill }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex gap-4">
          <div className="text-center">
            ...
            <div className="text-[10px] text-foreground/40 uppercase tracking-wider">WPM</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-sage tabular-nums">{accuracy}%</div>
            <div className="text-[10px] text-foreground/40 uppercase tracking-wider">Accuracy</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-foreground/30 tabular-nums">{drill.targetWpm}</div>
            <div className="text-[10px] text-foreground/40 uppercase tracking-wider">Goal</div>
          </div>
        </div>
      ...
      {/* Progress bar */}
      <div className="h-1 rounded-full bg-foreground/10 mb-4 overflow-hidden">
      ...
      <div
        ...
        className="cursor-text rounded-2xl border-2 border-foreground/10 bg-base/80 p-5 md:p-8 mb-4 min-h-[120px] transition-all duration-300 focus-within:border-amber/30 focus-within:shadow-lg focus-within:shadow-amber/5"
      >
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <p className="text-xs text-foreground/30">Click the text or press any key to start typing</p>
      </div>
      ...
      {completed && (
        <div className="mt-6 rounded-2xl bg-gradient-to-br from-amber/10 to-rust/10 border border-amber/20 p-6 text-center animate-scale-in">
          ...
          <div className="flex justify-center gap-6 mt-3">
            <div><span className="text-xl font-bold text-amber tabular-nums">{wpm}</span><span className="text-sm text-foreground/40 ml-1">WPM</span></div>
            <div><span className="text-xl font-bold text-sage tabular-nums">{accuracy}%</span><span className="text-sm text-foreground/40 ml-1">accuracy</span></div>
          </div>
    ...
    <div className="pt-24 pb-16">
      <div className="mx-auto max-w-6xl px-5">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-display font-bold text-foreground">Typing Mastery</h1>
          <p className="text-foreground/50 mt-1">Learn to type faster and more accurately — one drill at a time.</p>
        </div>
            ...
            <div className="card p-4 sticky top-24">
              <div className="flex gap-1 mb-4">
                {['All', 'Beginner', 'Intermediate', 'Advanced'].map((l) => (
                  <button key={l} onClick={() => setFilterLevel(l)} className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-all ${filterLevel === l ? 'bg-amber text-white' : 'bg-foreground/5 text-foreground/40 hover:text-foreground/70'}`}>
              ...
              <div className="space-y-1 max-h-[60vh] overflow-y-auto no-scrollbar">
                {filtered.map((d) => (
                  <button key={d._id} onClick={() => setActive(d)} className={`flex w-full items-center justify-between rounded-lg border px-3 py-2.5 text-left text-sm transition-all ${active?._id === d._id ? 'border-amber bg-amber/5 text-foreground' : 'border-transparent text-foreground/40 hover:text-foreground/70 hover:bg-foreground/5'}`}>
                    <span className="font-medium truncate mr-2">{d.title}</span>
                    <span className={`shrink-0 pill text-[10px] ${(levelColors)[d.level] || 'bg-foreground/5 text-foreground/30'}`}>{d.targetWpm}wpm</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div>
            {active ? (
              <div key={active._id} className="card p-5 md:p-8">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="text-xl font-display font-bold text-foreground">{active.title}</h2>
                    <span className={`inline-block mt-1 pill text-xs ${levelColors[active.level] || 'bg-foreground/5 text-foreground/30'}`}>{active.level}</span>
                  </div>
                </div>
                <Tester drill={active} />
              </div>
              ...
              <div className="card text-center py-16">
                ...
                <p className="text-foreground/50">Select a drill to start typing.</p>
              </div>
            )}
          </div>
        </div>
