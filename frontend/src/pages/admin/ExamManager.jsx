    ...
    <div className="pt-24 pb-16">
      <div className="mx-auto max-w-5xl px-5">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-display font-bold text-[#CFC89A]">Exams</h1>
        ...
        {exams.length === 0 ? (
          <div className="card p-12 text-center text-[#CFC89A]/30">No exams yet.</div>
        ) : (
          <div className="grid gap-3">
            {exams.map((ex) => (
              <div key={ex._id} className="card p-4 flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-[#CFC89A] font-medium truncate">{ex.title}</p>
                  <p className="text-[#CFC89A]/40 text-xs truncate mt-0.5">{ex.questions?.length || 0} questions · Level {ex.level || '?'}</p>
                </div>
