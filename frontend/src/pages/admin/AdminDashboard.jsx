      ...
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-foreground/50 mt-1">Manage your academy.</p>
          </div>
        ...
        {/* Tabs */}
        <div className="flex gap-1 border-b border-foreground/10 mb-6">
          {[
            { id: 'courses', label: 'Courses', icon: '📚' },
            { id: 'users', label: 'Users', icon: '👥' },
          ].map((t) => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} className={`flex items-center gap-1.5 px-4 py-3 text-sm font-medium border-b-2 transition-all ${activeTab === t.id ? 'text-amber border-amber' : 'text-foreground/40 border-transparent hover:text-foreground/70'}`}>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-foreground/5 border-b border-foreground/10">
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider">Course</th>
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider hidden md:table-cell">Category</th>
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider hidden sm:table-cell">Price</th>
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider text-right">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {courses.map((course) => (
                    <tr key={course._id} className="border-b border-foreground/5 hover:bg-foreground/[0.02] transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <span className="text-xl">{course.flag}</span>
                          <div>
                            <p className="font-semibold text-foreground text-sm">{course.title}</p>
                            <p className="text-xs text-foreground/30">{course.slug}</p>
                          </div>
                        </div>
                      </td>
                      <td className="p-4 text-sm text-foreground/50 hidden md:table-cell">
                        <span className="pill bg-foreground/5 text-foreground/50 text-[10px]">{course.category}</span>
                      </td>
                      <td className="p-4 text-sm text-foreground/60 hidden sm:table-cell">${course.price}</td>
          ...
          <div className="rounded-2xl border border-foreground/10 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-foreground/5 border-b border-foreground/10">
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider">Name</th>
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider hidden sm:table-cell">Email</th>
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider">Role</th>
                    <th className="p-4 text-xs font-semibold text-foreground/40 uppercase tracking-wider hidden md:table-cell">Joined</th>
                  </tr>
                </thead>
                <tbody>
                  {users.map((u) => (
                    <tr key={u._id} className="border-b border-foreground/5 hover:bg-foreground/[0.02] transition-colors">
                      <td className="p-4 text-sm text-foreground">{u.name}</td>
                      <td className="p-4 text-sm text-foreground/50 hidden sm:table-cell">{u.email}</td>
                      <td className="p-4">
                        <span className={`pill text-[10px] ${u.role === 'admin' ? 'bg-amber/10 text-amber' : u.role === 'teacher' ? 'bg-sage/10 text-sage' : 'bg-foreground/5 text-foreground/50'}`}>{u.role}</span>
                      </td>
                      <td className="p-4 text-xs text-foreground/30 hidden md:table-cell">{new Date(u.createdAt).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
