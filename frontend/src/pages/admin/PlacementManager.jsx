    ...
    <div className="pt-24 pb-16">
      <div className="mx-auto max-w-5xl px-5">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-display font-bold text-[#CFC89A]">Placement Test</h1>
        ...
        {placements.length === 0 ? (
          <div className="card p-12 text-center text-[#CFC89A]/30">No placement test yet.</div>
        ) : (
            ...
            {placements.map((p) => (
              <div key={p._id} className="card p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-[#CFC89A] font-medium">Placement Test</h3>
                    <p className="text-[#CFC89A]/40 text-xs">{p.questions?.length || 0} questions</p>
                  </div>
                  <div className="flex gap-2">
                ...
                <div className="space-y-2">
                  {p.questions?.slice(0, 3).map((q, i) => (
                    <div key={i} className="bg-[#322938] rounded p-3 text-sm text-[#CFC89A]/70">
                      <span className="text-amber text-xs mr-2">Q{i + 1}</span>
                      {q.question}
                    </div>
                  ))}
                  {(p.questions?.length || 0) > 3 && <p className="text-xs text-[#CFC89A]/30">+{p.questions.length - 3} more</p>}
                </div>
              </div>
            ))}
          </div>
