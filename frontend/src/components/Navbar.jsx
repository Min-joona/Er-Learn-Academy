  ...
  return (
    <header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? 'bg-base/90 backdrop-blur-2xl shadow-lg shadow-black/10' : 'bg-transparent'
      }`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 h-16 md:h-20">
        ...
        <div className="hidden md:flex items-center gap-1">
          <NavLink to="/" end className={({ isActive }) => `px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${isActive ? 'text-amber bg-amber/10' : 'text-foreground/60 hover:text-foreground hover:bg-foreground/5'}`}>
            Home
          </NavLink>

          {/* Mega Menu Trigger */}
          <div className="relative" onMouseEnter={() => setMegaOpen(true)} onMouseLeave={() => setMegaOpen(false)}>
            <button onClick={() => setMegaOpen((o) => !o)} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 flex items-center gap-1.5 ${location.pathname.startsWith('/courses') ? 'text-amber bg-amber/10' : 'text-foreground/60 hover:text-foreground hover:bg-foreground/5'}`}>
              Courses
            ...
            {megaOpen && (
              <div
                ref={megaRef}
                className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[600px] rounded-2xl border border-foreground/10 bg-base/95 backdrop-blur-2xl shadow-2xl shadow-black/30 p-5 animate-scale-in origin-top"
                ...
                <div className="grid grid-cols-2 gap-3">
                  {categories.map((cat) => (
                    <Link
                      ...
                      className="flex items-start gap-4 rounded-xl p-4 transition-all duration-200 hover:bg-foreground/5 group/card"
                    >
                      <span className="text-2xl shrink-0">{cat.icon}</span>
                      <div>
                        <p className="font-semibold text-foreground group-hover/card:text-amber transition-colors">{cat.name}</p>
                        <p className="text-xs text-foreground/40 mt-0.5">{cat.desc}</p>
                      </div>
                    </Link>
                  ))}
                </div>
          ...
          <NavLink to="/typing" className={({ isActive }) => `px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${isActive ? 'text-amber bg-amber/10' : 'text-foreground/60 hover:text-foreground hover:bg-foreground/5'}`}>
            Typing
          </NavLink>

          {user && (
            <NavLink to="/dashboard" className={({ isActive }) => `px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${isActive ? 'text-amber bg-amber/10' : 'text-foreground/60 hover:text-foreground hover:bg-foreground/5'}`}>
            Dashboard
            </NavLink>
          )}

          {user?.role === 'admin' && (
            <NavLink to="/admin" className={({ isActive }) => `px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${isActive ? 'text-rust bg-rust/10' : 'text-foreground/60 hover:text-rust hover:bg-rust/5'}`}>
              Admin
            </NavLink>
          )}

          {user?.role === 'teacher' && (
            <NavLink to="/teacher" className={({ isActive }) => `px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${isActive ? 'text-sage bg-sage/10' : 'text-foreground/60 hover:text-sage hover:bg-sage/5'}`}>
              Teacher
            </NavLink>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          {/* Theme toggle */}
          <button onClick={toggle} className="w-9 h-9 rounded-xl bg-foreground/5 hover:bg-foreground/10 flex items-center justify-center text-foreground/60 hover:text-amber transition-all" aria-label="Toggle theme">
            {dark ? (
          ...
          {user ? (
            <div className="hidden md:flex items-center gap-2">
              ...
              <Link to="/settings" className="w-8 h-8 rounded-lg flex items-center justify-center text-foreground/30 hover:text-amber transition-colors" title="Settings">
              ...
              <button onClick={handleLogout} className="px-3 py-2 text-sm text-foreground/50 hover:text-rust transition-colors" title="Logout">
            ...
            <div className="hidden md:flex items-center gap-2">
              <Link to="/login" className="px-4 py-2 text-sm font-medium text-foreground/60 hover:text-foreground transition-colors">Log in</Link>
              <Link to="/register" className="btn-primary py-2 px-4 text-sm !rounded-xl">Sign up</Link>
            </div>
          )}

          {/* Mobile toggle */}
          <button className="md:hidden w-9 h-9 rounded-xl bg-foreground/5 hover:bg-foreground/10 flex items-center justify-center text-foreground/60 hover:text-amber transition-all" onClick={() => setIsOpen(!isOpen)}>
            {isOpen ? (
      ...
      <div className={`md:hidden transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="border-t border-foreground/10 bg-base/95 backdrop-blur-2xl px-5 py-5 flex flex-col gap-1">
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-2 px-4 py-3 text-sm font-medium rounded-lg transition-colors ${
          isActive ? 'text-amber bg-amber/10' : 'text-foreground/60 hover:text-foreground hover:bg-foreground/5'
        }`
      }
    >
      {children}
